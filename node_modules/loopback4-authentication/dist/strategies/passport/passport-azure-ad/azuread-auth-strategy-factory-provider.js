...
let AzureADAuthStrategyFactoryProvider = class AzureADAuthStrategyFactoryProvider {
    ...
    createCallbackWithReq(verifyFn) {
        return async (req, iss, sub, profile, accessToken, refreshToken, done) => {
            if (!profile.oid) {
    ...
    createCallbackWithoutReq(verifyFn) {
        return async (iss, sub, profile, accessToken, refreshToken, done) => {
            if (!profile.oid) {
                return done(new Error('No oid found'), null);
            }
            try {
...
exports.AzureADAuthStrategyFactoryProvider = AzureADAuthStrategyFactoryProvider = tslib_1.__decorate([
